...
//    }
   ...
   if (!fs)
   {
    ...
    fs->f = last_perm = (struct url_actions *)mem_file_malloc(sizeof(*last_perm));
#else
   ...
   while (read_config_line(fp, &linenum, &buf) != NULL)
   {
      if (*buf == '{')
      {
         ...
         else
         {
            ...
            cur_action_used = 0;
             #ifdef IS_USE_MEMFILE
                 cur_action = (struct action_spec *)mem_file_malloc(sizeof(*cur_action));
             #else
      ...
      else if (mode == MODE_ALIAS)
      {
         ...
         if ((start == NULL) || (start == buf))
         {
          ...
          #ifdef IS_USE_MEMFILE
            if ((new_alias = mem_file_malloc(sizeof(*new_alias))) == NULL)
          #else
      ...
      else if (mode == MODE_ACTIONS)
      {
         ...
         /* allocate a new node */
          #ifdef IS_USE_MEMFILE
              if ((perm = mem_file_malloc(sizeof(*perm))) == NULL)
          #else
          ...
          if (!strcmpic(vec[0], "GEOIP") || !strcmpic(vec[0], "IP-CIDR") || !strcmpic(vec[0], "DNS-IP-CIDR")) {
              if (!strcmpic(vec[0], "GEOIP")) {
                  ...
                  if (!strcmpic(vec[0], "IP-CIDR")) {
                      // CIDR
                      #ifdef IS_USE_MEMFILE
                                size_t buffer_size =strlen(vec[1]);
                                char *ipcidr=mem_file_malloc(buffer_size);
                                strcpy(ipcidr, vec[1]);
                      #else
