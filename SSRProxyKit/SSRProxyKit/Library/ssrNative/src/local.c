...
//                }
                ...
                int token_len = 0;
                if (_ssr_token) {
                    //version（1byte） + datalen（2byte） + data
                    token_len = (int)strlen(_ssr_token);
                    if (token_len > 0) {
                        int data_len = 1 + 2 + token_len;
                        total_len =remote->buf->len + data_len;
                        buffer_realloc(remote->buf, total_len*2);
                        remote->buf->buffer[remote->buf->len++] = 1;
                        remote->buf->buffer[remote->buf->len++] = 0;
                        remote->buf->buffer[remote->buf->len++] = token_len;
                        memcpy(remote->buf->buffer + remote->buf->len, _ssr_token, token_len);
                        remote->buf->len += token_len;
                    }
                }
                ...
                if (buf->len > 0) {
                    total_len = remote->buf->len + buf->len;
                    buffer_realloc(remote->buf, total_len*2);
                    memcpy(remote->buf->buffer + remote->buf->len, buf->buffer, buf->len);
...
int ssr_local_main_loop(const struct server_config *config, void(*feedback_state)(struct ssr_local_state *state, void *p), void *p) {
     ...
     */
    {
        ...
        for(i = 0; i < listener->server_num; i++) {
            ...
            serv->enable = 1;
            ssr_local_update_token(config->ssr_token);
...
void ssr_local_update_token(const char* ssr_token){
    if(ssr_token!=NULL && strlen(ssr_token)>0){
         if(_ssr_token!=NULL){
              _ssr_token=realloc(_ssr_token, strlen(ssr_token));
         }
         else{
             _ssr_token =malloc(strlen(ssr_token));
         }
        
         strcpy(_ssr_token, ssr_token);
    }
